...
with
    ...
    , channel_cac as (
        ...
        left join campaign_cost as camp on c.acquisition_channel = camp.channel
...
select
    ...
    , round(
        cast(
            (m.aov * m.annual_freq * ${clv_expected_lifespan_years} * ${clv_gross_margin}) / nullif(cc.cac, 0) as double
        ), 2
    ) as ltv_to_cac_ratio
from metrics as m
